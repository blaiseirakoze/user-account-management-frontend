    ...
    return (
        <div className="p-2 bg-sky-900 text-white w-60 flex flex-col md:flex text-center" id="sideNav">
            <div className="text-center py-4">
                <h2 className="font-bold text-xl">DEMO</h2>
            </div>
                    ...
                    routes.map((route: any, index: number) => {
                        ...
                        return (
                            route?.visible === false ? null :
                                <Link key={index} className={`flex items-center py-2.5 px-4 my-4 rounded transition duration-200 hover:bg-gradient-to-r ${route.hidden || role == "CUSTOMER" ? "hidden" : ""} ${active ? "bg-sky-950" : ""} hover:from-sky-950 hover:to-sky-950 hover:text-white`} to={route.path}>
            ...
            <p className="mb-1 px-5 py-3 text-left text-xs text-cyan-500">Copyright DEMO@2023</p>
        </div>
    )
}
